...
class MainActivity : AppCompatActivity() {
    ...
    // Giữ lại trạng thái ping của màn hình để tự làm mới danh sách.
    // Cảnh báo đơn mới thực sự do OrderAlertService xử lý nền.
    ...
    private fun pingOnce() {
        ...
        thread {
            try {
                ...
                // Cảnh báo giọng nói + thông báo hệ thống được xử lý bởi
                // OrderAlertService để vẫn hoạt động khi app ra nền / khóa màn hình.
    ...
    private fun stopOrderAlertService() {
        ...
        getSharedPreferences("partner_order_alert", MODE_PRIVATE).edit().clear().apply()
    }
    ...
    private fun addPickup(o: JSONObject) {
        ...
        if (st == "pending") {
            // Luồng mới: Shop nhận đơn ngay khi khách đặt.
            // Shop chỉ cần báo "Món đã xong" khi hoàn tất; không bắt buộc bước "Nhận đơn" riêng.
            val row = LinearLayout(this)
            val ready = Button(this).apply { text = "🍱 Món đã xong — báo Shipper" }
            val no = Button(this).apply { text = "❌ Từ chối" }
            row.addView(ready, LinearLayout.LayoutParams(0, -2, 1f))
            row.addView(no, LinearLayout.LayoutParams(0, -2, 1f))
            c.addView(row)
            ready.setOnClickListener { action(id, "partner_mark_ready") { loadPickups("partner_pickups", "ready") } }
